...
try {
    ...
    $unlock_next = false;
    if (
        // ✅ Case 1: Quiz → must pass threshold AND chapter completed
        ($phase_type === 'quiz' && $newValue >= $quiz_pass_threshold && $chapter_percent === 100)
        ||
        // ✅ Case 2: Material/Video → must be 100% completed AND chapter completed
        (($phase_type === 'material' || $phase_type === 'video') && $newValue === 100 && $chapter_percent === 100)
    ) {
        ...
        if ($learning_type === 'sequential') {
            ...
            if ($nxRow = $nxRes->fetch_assoc()) {
                ...
                // Insert/update student_chapter_progress for next chapter
                ...
                if ($tmpRes->fetch_assoc()) {
                    ...
                    $i2 = $conn->prepare("INSERT INTO student_chapter_progress 
